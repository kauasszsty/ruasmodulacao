...
function newStreetPrint() {
	if (window.isDebug){
		window.newStreet = window.newStreet || [];
		var x = ""; 
		for (const c in window.newStreet) {
			x = x + "[" + Math.round( window.newStreet[c].x ) + "," + Math.round( window.newStreet[c].y ) + "], " ;
		} 
		console.log( x );
	}		
}		
function newStreetReset() {
	if (window.isDebug){
		window.newStreet = [];
		console.log( "STEET RESET." );
	}
}
